...
nhaptoantu2:
 ...
 Loop3: # Cho nhap vao dau = de hien thi ket qua  
    ...
    case_chia:  bne $s2,4,pheptinhdf
         	...
         	j showketqua
 pheptinhdf: # bao loi
  la $a0, err1	 #xay ra loi , hay nhap lai 
  li $v0, 4     
  syscall
...
showketqua: li $t9,0
            ...
            bne $k1,$0,loisoam
            div $t8,$s7,10 #chia s7 cho 10 luu ket qua vao t8, so du luu vao thanh ghi hi
            mfhi $t9 #luu so du sang t9
            beq $t8,0,napgiatricholed

            div $t8,$t8,10 #chia tiep t8 cho 10 de lay chu so hang chuc 
            mfhi $t8 
napgiatricholed: 
...
showleds_9: bne $s0,9,showleds_df # case $s0 = 9
      ...
      j napgiatri
showleds_df:  jr $ra
napgiatri:    sb $t4,0($t2) #lay ket qua tu t4 cho vao t2 de in ra 
	      jr $ra
...
getInt1: addi $sp,$sp,4
      ...
      lb $t3,0($t1)   # t3 : sau khi click thi gia tri se dc luu vao t3 
case_0:   li $t5,0x11 #
      bne $t3,$t5,case_1  # so sanh t3 vs t5
...
getInt1rt: lw $ra,0($sp)  #neu sai giai phong thanh ghi sp ,vi luc nay mo rong them 4 byte trong de luu tru 
      addi $sp,$sp,-4
      jr $ra
  #-------------------------------------------------------------------------
...
getInt4rt:       lw $ra,0($sp)
  ...
  #-------------------------------------------------------------------------
displayLED: addi $sp,$sp,4 #hien thi cac phim ra ben ngoai 
      sw $ra,0($sp)   # save $ra
display:    
...
assign:   sb $t4,0($t2)  #gan gia tri t4 vao t2
